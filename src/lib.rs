...
pub enum ClaimMode {
  /// Claim all pending entries from other consumers irregardless of min-idle-time until none remain while concurrently processing new entries until shutdown.
  /// This is useful for facilitating work-resumation for use cases in which there's guaranteed to be at most one consumer and a restart occurs; such case
  /// requires no idle-delay to claim pending work because it can be reasoned the old consumer is offline and has no pending work in progress.
  ClaimAllPending,
        ...
        if matches!(self.group_status, ConsumerGroupState::PreviouslyCreated) {
          try_join!(
            backoff::future::retry(ExponentialBackoff::default(), || async {
              self
                .process_idle_pending(&Duration::new(0, 0), &Some(Duration::new(0, 0)))
                .await?;
